...
| **Diagnóstico webhook en curso (2026-05-03)** | Pedidos de Shopify/EFI no aparecen en `/confirmacion`. Causa raíz aún no confirmada — se añadió logging detallado al webhook y endpoint de debug. Principales sospechosos: (A) `SHOPIFY_WEBHOOK_SECRET` desincronizado con Shopify → HMAC falla → 401 silencioso; (B) evento configurado en Shopify es `orders/paid` en vez de `orders/create` → pedidos COD no disparan webhook. Verificar usando `GET /api/debug/shopify-webhook-ingestion` y logs del servidor. | `api/webhooks/shopify/orders/route.ts`, `api/debug/shopify-webhook-ingestion/route.ts` (nuevo) |
