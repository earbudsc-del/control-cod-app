...
| **recover-shopify-orders: update de pedidos con datos incompletos (2026-05-04)** | 131 pedidos recuperados aparecían sin nombre/teléfono/dirección/ciudad. El endpoint solo insertaba nuevos y saltaba los existentes. Fix (paso 6 + paso 8): paso 6 ahora trae `id, customer_name, customer_phone, customer_address, city` de los existentes en un `Map`. Paso 8 se divide en Rama A (existe → revisa si alguno de esos 4 campos es null/vacío/"-" → UPDATE solo los campos vacíos + province/product_summary/cod_amount) y Rama B (no existe → INSERT igual que antes). Helper `isEmpty()` evalúa null, "", "-". Respuesta agrega `updated_missing_data`. Log: `[recover-diag] updated missing customer data: order_number`. | `src/app/api/admin/recover-shopify-orders/route.ts` |
...
| contiene "reparto" / "mensajero" / "en ruta" / "despacho" | `en_reparto` | |
...
- Procesa: `in_transit`, `en_reparto`, `novedad`, `unknown`
