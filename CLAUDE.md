...
| **recover-shopify-orders: búsqueda de tienda robusta (2026-05-04)** | El endpoint buscaba tienda solo por `is_active=true` sin filtrar por `shopify_domain`, ignorando el env var. Fix: paso 7 ahora busca primero `.eq('shopify_domain', SHOPIFY_SHOP_DOMAIN).eq('is_active', true)`. Si no hay match (domain no coincide o campo vacío), hace fallback a primera tienda activa. En ambos casos loguea la fuente de resolución. La respuesta 404 ahora incluye `shopify_domain_buscado`, `storeError` y `fallbackError` para diagnóstico. | `src/app/api/admin/recover-shopify-orders/route.ts` |
