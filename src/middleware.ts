...
// Rutas exclusivas para admin + dispatch_agent
const CONFIRMADOS_PATHS = ['/confirmados']
...
export async function middleware(request: NextRequest) {
  ...
  const needsRoleCheck =
    ...
    CONFIRMADOS_PATHS.some(p => path.startsWith(p))
  ...
  const canSeeConfirmados  = role === 'admin' || role === 'dispatch_agent'
  ...
  // Bloquear /confirmados para roles que no son admin ni dispatch_agent
  if (!canSeeConfirmados && CONFIRMADOS_PATHS.some(p => path.startsWith(p))) {
    return NextResponse.redirect(new URL('/my-tasks', request.url))
  }
